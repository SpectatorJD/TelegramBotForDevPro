        ...
        if (update.hasCallbackQuery()) {
            ...
            switch (callbackData) {
                ...
                case "query5_cat_button" -> {
                    SendMessage sendMessage = new SendMessage(String.valueOf(chatId), "вот информация по коту по выбору 5");
                    try {
                        execute(sendMessage);
                    } catch (TelegramApiException e) {
                        logger.info("Processing update: {}", update);
...
//                }
                ...
                case "query5_dog_button" -> {
                    SendMessage sendMessage = new SendMessage(String.valueOf(messageId), "вот информация по собаке по выбору 5");
                    try {
                        execute(sendMessage);
                    } catch (TelegramApiException e) {
                        logger.info("Processing update: {}", update);
    ...
    public void register2Cat(long chatId) {
        SendMessage message = new SendMessage();
        message.setChatId(String.valueOf(chatId));
        message.setText("choose: Консультация с потенциальным хозяином животного из приюта,because the rest is empty ");
        ...
        head2CatButton.setCallbackData("head2_cat_button");
    ...
    public void register2Dog(long chatId) {
        SendMessage message = new SendMessage();
        message.setChatId(String.valueOf(chatId));
        message.setText("choose: Консультация с потенциальным хозяином животного из приюта,because the rest is empty ");
        ...
        head2DogButton.setCallbackData("head2_dog_button");
    ...
    public void register3Cat(long chatId) {
        SendMessage message = new SendMessage();
        message.setChatId(String.valueOf(chatId));
        ;
        message.setText("choose, please");
    ...
    public void register3Dog(long chatId) {
        SendMessage message = new SendMessage();
        message.setChatId(String.valueOf(chatId));;
        message.setText("choose, please");
        ...
        query9DogButton.setText("Выдать рекомендации по проверенным кинологам для дальнейшего обращения к ним");
        query9DogButton.setCallbackData("query9_dog_button");

        var query10DogButton = new InlineKeyboardButton();
        query10DogButton.setText("Выдать список причин, почему могут отказать и не дать забрать собаку из приюта");
